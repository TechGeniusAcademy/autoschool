...
const CreateCourse: React.FC = () => {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // В реальном проекте здесь был бы загрузка на сервер
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || "Ошибка при создании курса");
      }
    } catch (error) {
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="mt-2 text-gray-600">Заполните информацию о новом курсе</p>
              </div>
              <button onClick={() => router.push("/admin/courses")} className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                <ArrowLeft className="w-5 h-5" />
                Назад
              </button>
            </div>
