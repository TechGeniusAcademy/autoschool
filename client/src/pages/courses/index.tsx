...
const CoursesPage: React.FC = () => {
  ...
  const fetchCourses = async (page = 1) => {
    try {
      ...
      // Используем публичный эндпоинт для загрузки курсов
      const response = await fetch(`http://localhost:3001/api/courses/public?${params}`, { headers });
      ...
      if (data.success) {
        ...
        setError(data.message || t("error_loading_courses"));
      }
    } catch (error) {
  ...
  const handleEnrollment = async (courseId: number, courseTitle: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || t("enrollment_error"));
      }
    } catch (error) {
      console.error(t("enrollment_error"), error);
      alert(t("enrollment_error"));
    }
  };
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Заголовок */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900">{t("all_courses")}</h1>
            <p className="mt-2 text-gray-600">{t("courses_description")}</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder={t("course_search_placeholder")} className="pl-10 w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-red-500 focus:border-red-500" />
                </div>
              </div>
              ...
              <div>
                ...
                <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-red-500 focus:border-red-500">
                  ...
                  <option value="theory">{t("theory")}</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t("filter_difficulty")}</label>
                <select value={difficultyFilter} onChange={(e) => setDifficultyFilter(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-red-500 focus:border-red-500">
                  <option value="">{t("all_difficulty")}</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t("filter_instructor")}</label>
          ...
          {courses.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">{t("no_courses_found")}</h3>
                ...
                {courses.map((course) => (
                  <div key={course.id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-2 mb-4">
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              {course.students_count} {t("students")}
                            </span>
                          </div>
                          {course.rating && (
                            <div className="flex items-center gap-1">
                      ...
                      {/* Цена */}
                      <div className="mb-4">
                        <span className="text-2xl font-bold text-red-600">{course.price > 0 ? `${course.price} ₸` : t("free")}</span>
                      </div>
